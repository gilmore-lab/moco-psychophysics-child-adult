...
### Mean accuracy by pattern and coherence
...
df_p2_c_acc_pat_coh$Coh <-
  factor(
    df_p2_c_acc_pat_coh$Coh,
    levels = c("0.15", "0.3", "0.45", "0.6"),
    labels = c("15%", "30%", "45%", "60%")
  )
df_p2_c_acc_pat_coh$Pattern <-
  factor(
    df_p2_c_acc_pat_coh$PatternType,
...
p0 <-
  ...
  ggtitle("Children: Accuracy by coherence & pattern") +
  xlab("Coherence") +
  ylim(45, 100) +
...
### Mean accuracy by gender and pattern
...
p3 <-
  ...
  geom_point(
    ...
    aes(x = Gender, y = Pct.Corr.mean, color = Pattern),
    size = 3,
    position = position_dodge(0.8)
  ) +
  geom_errorbar(
  ...
  geom_jitter(position = position_dodge(0.8)) +
  ggtitle("Children: Accuracy by sex & pattern") +
  theme.custom + # Position legend in bottom right
  ylab("Mean Accuracy (%)") +
  ylim(45, 100) +
  scale_color_viridis(discrete = TRUE) +
...
ggsave(
  plot = p_c,
  filename = paste0(here::here(), "/analysis/pub_image/child.png"),
  units = "in",
  height = 12,
  width = 12,
  dpi = 600
