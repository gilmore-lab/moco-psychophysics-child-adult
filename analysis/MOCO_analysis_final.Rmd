...
```
```{r acc-logit-full-child, eval=FALSE}
form_c.explor3<- Acc ~ scale(Coh)+Speed+PatternType+Gender+scale(Age)+scale(Coh)*Speed+ Speed*scale(Age)+ (1|SubID)
model.acc.logit_c <- glmer(formula = form_c.explor3, family=binomial(mafc.logit(2)), glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)), data = df_child)
summary(model.acc.logit_c)
car::Anova(model.acc.logit_c, model.acc.logit.ex2_c)
```
