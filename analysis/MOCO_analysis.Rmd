...
# get coefficient
# str(df_child)
form.acc.logit2<- Acc ~ Coh + Speed + PatternType + Age + Gender + Coh*Speed + +Age*Speed+ (1|SubID) #Age*Coh design related
...
p1_c_acc <- 
  ggplot(data=df_p1_c_acc , aes(x=Coh, y=Pct.Corr.mean*100, color = AgeYrs) ) +
...
```{r }
df_p1_c_acc_speed_coh <- df_child %>% 
  ...
  group_by(Speed, Coh) %>%
  summarize(Pct.Corr.mean = mean(Pct.Corr, na.rm=TRUE),
            Pct.Corr.sem = sd(Pct.Corr, na.rm=TRUE)/sqrt(30))
...
df_p1_c_rt <- df_child %>% 
  filter(Group=="linear-radial coh [.15 .3 .45 .6]") %>%
  group_by(PatternType,Speed,Coh, AgeYrs) %>% 
  summarize(RT.mean=mean(RT, na.rm = T),
            RT.se=sd(RT, na.rm = T)/sqrt(25))
limits = aes( ymax = RT.mean + RT.se , ymin = RT.mean - RT.se )

p1_c_rt <- 
  ggplot(data=df_p1_c_rt , aes(x=Coh, y=RT.mean, color = AgeYrs) ) +
  facet_grid( facets = PatternType ~ Speed ) +
  geom_line() +
  geom_pointrange(limits) +
  xlim(0.15,0.60) +
...
df_all$Speed <- factor(df_all$Speed, labels = c("2 deg/s", "8 deg/s"))
df_all$SubID <- factor(df_all$SubID)
...
df_child2 <- df_child %>%
   dplyr::select(SubID, PatternType, RT, Acc, Speed, Gender, Coh, Response, AgeYrs) %>%
...
df.sdt_c$SubID<-as.integer(df.sdt_c$SubID) 
