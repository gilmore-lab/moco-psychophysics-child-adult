...
tab <- cbind(Est = fixef(model.acc.logit), LL = fixef(model.acc.logit) - 1.96 * se, UL = fixef(model.acc.logit) + 1.96 * se)
```
...
```
For a continuous outcome where we assume a normal distribution, the most common link function is simply the identity. It essentially drops out and we are back to our usual specification of means and variances for the normal distribution, which is the model used for typical linear mixed models. 
...
```{r mixed-effects-survival-model-RT}
m <- coxph(survobj ~ Age + Sex + Speed+PatternType+Coh + cluster(SubID), 
    data = df2)
## summary of the model
summary(m)
# given repeated observations on individuals each individual likely has their own baseline so adjust with a mixed effects cox model using the coxme package
m2 <- coxme(survobj ~ Age + Sex + Speed+PatternType+Coh + cluster(SubID), 
    data = df2)
m2
```
