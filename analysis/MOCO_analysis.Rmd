...
df_all <- read.csv(file="~/R/moco-psychophysics/analysis/CombinedData.csv", header=TRUE, sep=",")
...
ks.test(df_child$RT,y='pnorm',alternative='two.sided')
shapiro.test(df_child$RT)
Note that the rt is not normally distributed, which violated the assumption of mixed effect model.
So, use nonlinear regression
1. log transformation of DV
```{r rt-no-gaussian-log, eval=FALSE}
form.fixed.rt3 <- log(RT)~ Coh + Speed + PatternType + (1|SubID)
model.fixed.rt3 <- lmer(formula = form.fixed.rt3, data = df_child, na.omit, REML=FALSE)
summary(model.fixed.rt3)
```
2. nonlinear model
```{r rt-nonlinear, eval=FALSE}
model.nlmer.rt <- nlmer(formula = form.fixed.rt3, data = df_child, na.omit, REML=FALSE)
summary(model.nlmer.rt)
```
### Gaussian model with random intercept
```{r rt-gaussian, eval=FALSE}
# normalize to mean=0, sd=1
RT.norm <- (df_child$RT - mean(df_child$RT,na.rm=T)) / sd(df_child$RT,na.rm = T)
...
model.fixed.rt2 <- lmer(formula = form.fixed.rt2, data = df_child, REML=FALSE)
...
model.log.rt <- lmer(formula = form.log.rt, data = df_child, REML=FALSE)
...
form.interaction.rt<- log(RT) ~ Coh + Speed + PatternType  + Age+ Gender + Coh*Speed*PatternType+ (1|SubID)
model.interaction.rt <- lmer(formula = form.interaction.rt, data = df_child, REML=FALSE)
summary(model.interaction.rt) #AIC 23507
```

```{r rt-interaction-full_adult, eval=FALSE}
form.log.rt2_a<- log(RT) ~  Gender+Coh+ PatternType +Speed+ PatternType*Coh*Speed+ (1|SubID)
model.log.rt2_a <- lmer(formula = form.log.rt2_a, data = df_adult, REML=FALSE)
summary(model.log.rt2_a) #AIC 14131
car::Anova(model.log.rt2_a) # in package car
```
```{r rt-interaction-full_adult, eval=FALSE}
form.log.rt2_a<- log(RT) ~  Gender+Coh+ PatternType +Speed+ PatternType*Coh+Coh*Speed+ (1|SubID)
model.log.rt2_a <- lmer(formula = form.log.rt2_a, data = df_adult, REML=FALSE)
summary(model.log.rt2_a) #AIC 13639.4
car::Anova(model.log.rt2_a) # in package car
```
```{r rt-interaction-nolog_adult, eval=FALSE}
form.log.rt3_a<- RT ~  Gender+Coh+ PatternType +Speed+ PatternType*Coh+Coh*Speed+ (1|SubID)
model.log.rt3_a <- lmer(formula = form.log.rt3_a, data = df_adult, REML=FALSE)
summary(model.log.rt3_a) #AIC 34259
car::Anova(model.log.rt3_a) # in package car
```

```{r get-df}
anova(model.log.rt2_a) # does not give df
lmerTest::anova(model.log.rt2_a)
```

```{r get-95%CI&OR-child}
cc_a <- confint(model.log.rt2_a)  ## slow (~ 11 seconds)
# get oR and 95% CI
ctab_a <- cbind(OR=fixef(model_a.acc.logit),cc_a)
# Exponentiate to get odds ratios:
rtab_a <- exp(ctab_a)
