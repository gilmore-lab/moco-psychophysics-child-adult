...
```{r get-df}
# get df. One way afex does not work for glmer, try lmer to get df, which should be the same as the glmer
car::Anova(model.log.rt2,type=3)
car::Anova(model.log.rt2,ddf="Kenward-Roger")
```
```{r get-95%CI&OR-child}
...
form.acc.logit<- Acc ~ Gender + Coh + Speed + PatternType + group + group*Coh+group*PatternType+group*Speed+group*Gender + (1|SubID) 
...
summary(model.acc.logit_lmer)
anova(model.acc.logit_lmer)
afex::mixed(Acc ~ Gender + Coh + Speed + PatternType + group + group*Coh+group*PatternType+group*Speed+group*Gender + (1|SubID) ,family=binomial, data=df_all, method = "PB")
```