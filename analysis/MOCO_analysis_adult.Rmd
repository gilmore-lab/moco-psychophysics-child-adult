...
df <- read.csv(file = "~/moco-psychophysics-child-adult/analysis/data/adult/adult-laminar-radial-grouped.csv", header = TRUE,sep=",")

# Convert gender into dummy
df$Age<-df$AgeYears
df$SubID<-df$ParticipantID
df$Acc<-as.logical(df$Acc == 1)
df$Speed<-df$DegPSec
# df$Speed <- factor(df$DegpSec, levels=c(2,8), labels = c("2deg/s", "8deg/s"))
...
hist(df.clean$RT,breaks=100)
qqPlot(df.clean$RT)
qqnorm(df.clean$RT); qqline(df.clean$RT)
# lognormal
qqp(df2$RT,"lnorm")
...
summary(model.acc.full) # AIC 9414
anova(model.acc.fixed,model.acc.full)  # significant
anova(model.acc.full)
```
#### interaction effect
...
summary(model.acc.full2) # AIC 9351.8
```
```{r acc-probit-interaction-gender-age, eval=FALSE}
form.full3<- Acc ~ Coh + Speed + PatternType + Gender + Coh*Speed + Coh*PatternType +Gender*Coh + Gender*Speed + Gender*PatternType+ (1|SubID)
model.acc.full3 <- glmer(formula = form.full3, family=binomial(mafc.probit(2)), data = df2) AIC 9359.9
summary(model.acc.full3)
form.full4<- Acc ~ Coh + PatternType+Speed + PatternType*Coh +Gender*Speed + Gender*PatternType+ (1|SubID)
model.acc.full4 <- glmer(formula = form.full4, family=binomial(mafc.probit(2)), data = df2)
anova(model.acc.full3,model.acc.full4)  # 9356.9, better fit
```
...
model.acc.logit <- glmer(formula = form.full4, family=binomial(mafc.logit(2)), data = df2)
summary(model.acc.logit) # better fit than probit
anova(model.acc.logit,model.acc.full4)
form.logit<-Acc ~ Coh + Speed + PatternType + Gender+  Coh*PatternType +  (1|SubID)
model.acc.logit <- glmer(formula = form.logit, family=binomial(mafc.logit(2)), data = df2)
...
summary(model.empty.rt) # AIC 37178.1
```
ICC_RT <- 
...
summary(model.fixed.rt3) # 14339.5
```
...
form.log.rt<- log(RT) ~  Gender +  Coh *Speed *PatternType + (1|SubID)
model.log.rt <- lmer(formula = form.log.rt, data = df2, REML=FALSE)
summary(model.log.rt) #AIC 14246.6
car::Anova(model.log.rt)
```

```{r rt-interaction-full, eval=FALSE}
form.log.rt2<- log(RT) ~  Gender+Coh+ PatternType +Speed+ PatternType*Coh+Coh*Speed+ (1|SubID)
model.log.rt2 <- lmer(formula = form.log.rt2, data = df2, REML=FALSE)
summary(model.log.rt2) #AIC 14131
car::Anova(model.log.rt2) # in package car
